...
class MarkdownTabView {
    ...
    createContent() {
        ...
        return {
            ...
            copyMarkdown: documentActions.copyMarkdown,
            copyMarkdownLabel: documentActions.copyMarkdownLabel,
    ...
    createDocumentActions() {
        ...
        const copyMarkdown = this.createElement('button', {
            id: 'mktero-copy-markdown',
            class: 'markdown-reader-action markdown-reader-action--child',
            type: 'button',
            'aria-label': this.t('viewer.copyMarkdown'),
            title: this.t('viewer.copyMarkdown'),
        });
        copyMarkdown.appendChild(createLucideIcon(
            this.document,
            LUCIDE_ICONS.copy,
            {
                className: 'markdown-reader-action-icon',
        ...
        const copyMarkdownLabel = this.createElement(
            'span',
            { class: 'markdown-reader-action-label' },
            this.t('viewer.copyMarkdownShort')
        );
        copyMarkdown.appendChild(copyMarkdownLabel);
    ...
    bindActions() {
        ...
        this.listen(this.elements.copyMarkdown, 'click', () => {
            this.runDocumentAction('copyMarkdown', 'onCopyMarkdown');
        });
    ...
    runDocumentAction(kind, callbackName) {
        const button = {
            ...
            copyMarkdown: this.elements.copyMarkdown,
        ...
        try {
            ...
            this.zotero?.logError?.(error);
            this.reportDocumentActionError(kind, error);
            this.documentActionBusy = null;
            this.syncDocumentActions(
                this.model,
                createLoadingPresentation(this.model, this.t)
        ...
        Promise.resolve(operation)
            .then(result => {
                ...
                else if (kind === 'copyMarkdown') {
                    this.setDocumentActionStatus(
                        'viewer.markdownCopied',
                        { dismissAfter: true }
                    );
                }
                else if (kind === 'exportMarkdown' && !result?.cancelled) {
                    this.setDocumentActionStatus(
                        'viewer.exportSaved',
                        { dismissAfter: true }
                    );
    ...
    reportDocumentActionError(kind, error) {
        if (kind === 'saveSnapshot') {
            this.setDocumentActionStatus(
            ...
            this.setDocumentActionStatus(
                'viewer.markdownCopyFailed',
                { dismissAfter: true }
            );
        }
        else if (kind === 'exportMarkdown') {
            this.setDocumentActionStatus(
                'viewer.exportFailed',
                { dismissAfter: true }
            );
    ...
    syncLocalization() {
        ...
        this.elements.copyMarkdown.setAttribute(
            'aria-label',
            this.t('viewer.copyMarkdown')
        );
        this.elements.copyMarkdown.setAttribute(
            'title',
            this.t('viewer.copyMarkdown')
        );
        this.elements.copyMarkdownLabel.textContent = this.t(
            'viewer.copyMarkdownShort'
        );
    ...
    syncDocumentActions(model, loadingView) {
        ...
        const saveAvailable = typeof model.onSaveSnapshot === 'function'
            && model.renderMode !== 'html';
        const copyAvailable = typeof model.onCopyMarkdown === 'function'
            && model.status === 'ready'
            && model.renderMode !== 'html';
        const exportAvailable = typeof model.onExportMarkdown === 'function'
            && model.status === 'ready'
            && model.renderMode !== 'html';
        ...
        this.elements.copyMarkdown.hidden = !copyAvailable;
        ...
        this.elements.saveSnapshot.disabled = !saveAvailable
            || loadingView.visible
            || Boolean(this.documentActionBusy);
        this.elements.copyMarkdown.disabled = !copyAvailable
            || loadingView.visible
            || Boolean(this.documentActionBusy);
        ...
        this.elements.copyMarkdown.setAttribute(
            'aria-busy',
            String(this.documentActionBusy === 'copyMarkdown')
        );
        this.elements.exportMarkdown.setAttribute(
            'aria-busy',
            String(this.documentActionBusy === 'exportMarkdown')
        );
        if (!available) {
            this.documentActionsOpen = false;
        }
    ...
    syncDocumentActionMenuState(visible) {
        ...
        this.elements.copyMarkdown.setAttribute('tabindex', menuTabIndex);
