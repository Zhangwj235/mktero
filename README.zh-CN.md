...
在更多菜单中选择 `校对识别结果`，然后双击段落、标题或 GFM 表格单元格。段落和标题
使用 `Ctrl/Command+Enter` 保存，`Escape` 取消当前修改；表格单元格在离开单元格时
保存。每个已校对内容块都可以单独恢复，也可以通过更多菜单恢复全部校对。
...
- Markdown 默认只读。校对模式只能替换已有段落、标题和 GFM 表格单元格的内容，不能
  改变文档结构、公式、图片或原始 HTML；标注操作与 Markdown 校对相互独立。
