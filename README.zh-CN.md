...
原始结果；可选的 AI 翻译会按一级标题并发翻译整篇文章，并在原文、译文和按块对比阅读之间切换。
...
> AI 翻译是独立的数据发送路径，会按一级标题切分后发送给所配置的 Provider，最多同时执行 5 个请求。
...
| 启用 AI 功能 | 关闭 | 通过配置的模型服务翻译 Markdown 文章（按一级标题切分，最多 5 个并发） |
| 流式响应 | 开启 | 流式接收每个分段响应；关闭后等待每段响应结束 |
...
| 请求超时 | 600,000 毫秒 | 单个 Markdown 分段请求最长可等待一小时 |
...
4. 使用 Markdown 正文上方的固定工具栏调整字号和字体、切换阅读模式或翻译全文；这些控件
   使用紧凑且一致的间距，翻译按钮与阅读模式之间带有分割线。右侧更多菜单包含
...
`翻译全文`。Mktero 会按 Markdown 一级标题切分经过保护的文档，最多并发发送 5 个分段请求，
并在所有分段通过结构校验后按原始顺序合并。请求前，代码、图片、独立公式、链接定义、原始
HTML、URL 和行内代码会被替换为受保护占位符；只有响应通过结构校验后才会恢复。翻译只会在用户明确
点击后开始，不会改写原始 Markdown，也不会进入保存的快照。默认使用流式传输，但阅读器
只在所有分段通过校验后更新，不会按每个 token 重绘全文。翻译进行时，工具栏按钮会显示
旋转的加载图标，并切换为“取消翻译”；再次点击即可停止当前请求。
...
- 通过配置的 Vercel AI SDK Provider 按一级标题并发翻译整篇 Markdown，并支持原文、译文和按块对比阅读。
...
| 用户选择全文翻译后按一级标题切分的受保护 Markdown 分段 | 配置的 AI Provider | Mktero 不同步 |
...
不会发送给 MinerU 或 AI Provider。全文翻译会按一级标题切分，并向配置的 AI Provider 并发发送最多 5 个
经过保护的 Markdown 分段和翻译指令。“复制并附带来源”只读取本地条目元数据，并将生成的结果
写入系统剪贴板。
