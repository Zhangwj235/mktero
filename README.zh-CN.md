...
在普通阅读界面中直接双击段落、标题或 GFM 表格单元格即可编辑。段落和标题可通过
内容块操作栏或 `Ctrl/Command+Enter` 保存，通过操作栏或 `Escape` 取消；表格单元格
在离开单元格时保存。链接、图片、引用和批注等交互元素保留原有的双击行为。内容块操作栏
还可以直接删除整个段落或标题，无需逐字删除。
