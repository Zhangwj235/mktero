...
原始结果；可选的 AI 翻译会按一级标题分组并拆成受控批次并发翻译整篇文章，并在原文、译文和上下分屏对比阅读之间切换。
...
只在所有批次处理完成后更新，不会按每个 token 重绘全文。翻译进行时，工具栏按钮会显示
旋转的加载图标，并切换为“取消翻译”；再次点击即可停止当前请求。

单独重试后仍然失败的内容块会保留原文，并将结果标记为可重试的“部分完成”；已经成功的译文仍可阅读。

翻译完成后，可以在工具栏选择 `阅读原文`、`阅读译文` 或 `对比阅读`。每次打开文档默认
显示原文；对比阅读会将原文置于上方、译文置于下方，作为两个平等且可独立滚动的阅读面，
并只保留一份原文目录。即使 Zotero 侧栏占用正文宽度，两种文本也会同时可见。由于译文不再
共享原 Markdown 的字符位置，PDF 标注和来源跳转仍只关联原文。
...
- 通过配置的 Vercel AI SDK Provider 使用受控批次并发翻译整篇 Markdown，并支持原文、译文和上下分屏对比阅读。
