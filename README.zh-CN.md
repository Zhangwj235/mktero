...
原始结果；可选的 AI 翻译会按一级标题分组并拆成受控批次并发翻译整篇文章，并在原文、译文和双栏对比阅读之间切换。
...
显示原文；对比阅读会将原文和译文作为两个平等、可独立滚动的连续阅读面并排显示，并只保留
一份原文目录；窗口较窄时，两个阅读面会改为上下排列。由于译文不再共享原 Markdown 的字符
位置，PDF 标注和来源跳转仍只关联原文。
...
- 通过配置的 Vercel AI SDK Provider 使用受控批次并发翻译整篇 Markdown，并支持原文、译文和双栏对比阅读。
