...
转换本地 PDF，并在只读 Zotero 标签页中展示 Markdown 正文、公式、表格、图片、
学术引用和标注。
...
| Body text size | 18 px | 在 16–22 px 间调整 Markdown 和快照字号 |
...
- 将 OCR 结果、双栏正文、公式、表格、图片、列表和代码重排成连续的只读文档。
...
- Markdown 文档内容为只读。标注操作会先创建本地记录，并在可靠匹配后同步到 Zotero
  PDF 标注，不会编辑转换后的 Markdown。
