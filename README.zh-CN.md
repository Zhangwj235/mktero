...
`导出 Markdown` 会打开系统“另存为”窗口，将当前修正后的原文 Markdown 写入选定的本地路径。提取图片会写入新的同级 `<文件名>.assets` 目录，Markdown 中的图片路径会同步更新。已存在的资源目录不会被覆盖，Mktero 会改用带序号的新目录。首版不导出译文或双语阅读视图，也不会自动导出。
