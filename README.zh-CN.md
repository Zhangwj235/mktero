...
> AI 翻译是独立的数据发送路径，会将正文拆成受控批次后发送给所配置的 Provider，最多同时执行 5 个请求。
...
| 翻译语言 | 简体中文 | 可选择简体中文、繁体中文、英文、日文、韩文、西班牙语、法语或葡萄牙语（巴西）；已打开的 Markdown 标签页会优先复用所选语言的完整缓存 |
...
4. 使用 Markdown 正文上方的固定工具栏调整字号和字体、切换阅读模式或翻译全文；这些控件
   ...
   点击该 Tab 可以在当前论文已经完整缓存的译文语言之间切换；缺失或不完整的语言不会显示。
...
按原始顺序合并；缺失或无效的内容块最多单独重试 2 次，参考文献区域保持原样。请求前，代码、图片、独立公式、链接定义、原始
HTML、URL 和行内代码会被替换为受保护占位符；只有响应通过结构校验后才会恢复。翻译只会在用户明确
...
请求。后台重试期间，已有的原文、译文和双语视图仍可继续阅读和切换。
...
高亮，也不提供针对单个内容块的翻译操作；更多菜单中的 `重新翻译全文` 会按当前设置重新生成
整篇译文。
...
目标语言和 Prompt 版本区分，每种目标语言保留独立结果。设置中切换语言时，Mktero 会先保持
当前阅读内容和阅读模式，再检查新语言的缓存；完整缓存会立即替换标签页中的译文，缺失或
部分缓存不会替换当前译文。只要已有完整译文可读，主工具栏就不再显示翻译按钮；部分译文
仍会显示重试操作，需要生成其他语言时可使用更多菜单中的 `重新翻译全文`。部分缓存会继续
补译缺失内容块，不会被误当成完整译文。
译文阅读模式 Tab 只列出当前论文和模型配置下的完整缓存语言；选择后会立即切换可见译文，
不会修改设置中的默认翻译语言，也不会再次请求 AI。阅读时会按原始顺序从缓存内容块重建
...
| 用户选择全文翻译后拆分的受保护 Markdown 批次 | 配置的 AI Provider | Mktero 不同步 |
...
经过保护的 Markdown 批次和翻译指令。“复制并附带来源”只读取本地条目元数据，并将生成的结果
写入系统剪贴板。
